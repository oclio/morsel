...
export async function resolveExtends(
  ...
  return resolveExtendsRecursive(filePath, options, visited, 0);
}

async function resolveExtendsRecursive(
  filePath: string,
  options: ResolveExtendsOptions,
  visited: Set<string>,
  depth: number,
  ...
  const fileResult = await loadFile(resolvedPath, options.formatPlugins);
  ...
  const parentResults: ResolveExtendsResult[] = [];
  for (const parentPath of processed.parentPaths) {
    parentResults.push(
      await resolveExtendsRecursive(
        parentPath,
        options,
        branchVisited,
        depth + 1,
